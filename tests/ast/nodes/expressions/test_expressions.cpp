...
TEST_CASE("Binary expression: addition", "[expressions][binary][arithmetic]")
{
    ...
    auto *left = std::get_if<ast::TokenExpr>(bin->left.get());
    REQUIRE(left != nullptr);
    REQUIRE(left->text == "5");

    auto *right = std::get_if<ast::TokenExpr>(bin->right.get());
...
TEST_CASE("Call expression: single argument", "[expressions][call]")
{
    ...
    auto *arg = std::get_if<ast::TokenExpr>(call->args.get());
    REQUIRE(arg != nullptr);
...
TEST_CASE("Call expression: multiple arguments", "[expressions][call]")
{
    ...
    auto *args = std::get_if<ast::GroupExpr>(call->args.get());
...
TEST_CASE("Call expression: slice with range", "[expressions][call][slice]")
{
    ...
    auto *range_expr = std::get_if<ast::BinaryExpr>(call->args.get());
...
TEST_CASE("Complex expression: nested binary operations", "[expressions][complex]")
{
    ...
    auto *mul = std::get_if<ast::BinaryExpr>(add->right.get());
    REQUIRE(mul != nullptr);
    REQUIRE(mul->op == "*");
}
...
TEST_CASE("Complex expression: function call with arithmetic", "[expressions][complex]")
{
    ...
    auto *arg = std::get_if<ast::BinaryExpr>(call->args.get());
    REQUIRE(arg != nullptr);
    REQUIRE(arg->op == "+");
}
