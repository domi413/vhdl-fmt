...
TEST_CASE("GenericParam: Mixed types", "[declarations][generic]")
{
    constexpr std::string_view VHDL_FILE = R"(
        entity E is
            generic (
    ...
    REQUIRE(entity->generic_clause.generics[0].type_name == "positive");
    REQUIRE(entity->generic_clause.generics[1].type_name == "time");
    REQUIRE(entity->generic_clause.generics[2].type_name == "string");
}
