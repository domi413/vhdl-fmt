...
struct BinaryExpr;
struct ParenExpr;
struct FunctionCallOrIndexedNamePart;
...
using Expression = std::
  variant<TokenExpr, GroupExpr, UnaryExpr, BinaryExpr, ParenExpr, FunctionCallOrIndexedNamePart>;
...
{
    std::vector<Expression> children; ///< Ordered child expressions.
};
...
struct UnaryExpr : NodeBase
{
    ...
    Box<Expression> value; ///< Operand expression (boxed for recursion).
};

/// Binary expression (e.g. `a + b`, `x downto 0`).
struct BinaryExpr : NodeBase
{
    Box<Expression> left;  ///< Left operand (boxed for recursion).
    std::string op;        ///< Binary operator symbol.
    Box<Expression> right; ///< Right operand (boxed for recursion).
};

/// Explicit parentheses around an expression (e.g. `(a + b)`).
struct ParenExpr : NodeBase
{
    Box<Expression> inner; ///< Inner expression inside parentheses (boxed for recursion).
};
...
struct FunctionCallOrIndexedNamePart : NodeBase
{
    Box<Expression> callee; ///< Function/array name being called/indexed.
    Box<Expression> args;   ///< Arguments (single expr or GroupExpr for multiple args).
};
